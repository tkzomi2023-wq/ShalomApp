...
export const MemberIDCardModal: React.FC<MemberIDCardModalProps> = ({ member, isOpen, onClose }) => {
  ...
  // Click outside to close save image(s) menu dropdown
  useEffect(() => {
  ...
  // Generate QR Code that links directly to the member's profile page
  useEffect(() => {
    if (isOpen && member) {
      // Build a full profile URL using the current origin and path with the query parameter
      const profileUrl = `${window.location.origin}${window.location.pathname}?profile=${member.id}`;
          ...
          <div className="bg-stone-100 dark:bg-stone-950 p-4 border-b border-stone-200 dark:border-stone-850 flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative" ref={saveMenuRef}>
                ...
                {isSaveMenuOpen && (
                  <div className="absolute right-0 mt-1.5 w-44 bg-white dark:bg-stone-850 rounded-xl shadow-xl border border-stone-200 dark:border-stone-750 py-1 z-50 animate-in fade-in slide-in-from-top-1 duration-150">
                    <button 
                      type="button"
                      onClick={() => {
                        downloadFrontAsPng();
                      ...
                      className="w-full text-left px-3.5 py-2 text-xs font-semibold text-stone-700 dark:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-800 cursor-pointer flex items-center justify-between"
                    >
                    ...
                    <button 
                      ...
                      className="w-full text-left px-3.5 py-2 text-xs font-bold text-emerald-600 dark:text-emerald-400 hover:bg-stone-50 dark:hover:bg-stone-800 cursor-pointer flex items-center justify-between"
                    >
                      <span>Download Both PNGs</span>
                    </button>
                  </div>
                )}
              </div>
